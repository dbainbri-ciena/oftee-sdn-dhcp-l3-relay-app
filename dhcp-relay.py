...
# Not real great practice for x-thread comms, but this is a hack
xids = {}

# Process a DHCP reponse packet  and packet out a response
def process(p):
    if scapy.BOOTP in p and p[scapy.UDP].dport == 68:

        # Lookup DPID and port based on transaction ID
        if int(p[scapy.BOOTP].xid) not in xids:
            print "UNEXPECTED/UNKNOWN XID: '%s'" % int(p[scapy.BOOTP].xid)
            sys.stdout.flush()
            return
...
# Thread used to sniff for DHCP responses and send them to a processor
def sniffer():
    scapy.sniff(filter="udp and dst port 68", prn=process)
...
def packet_in():
    ...
    # store the XIX mapping to the worker (response) thread
...
if __name__ == "__main__":
    capture = t.Thread(target=sniffer)
    capture.daemon = True
    capture.start()
